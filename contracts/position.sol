...
import "./lib/utils/structs/EnumerableSet.sol";
import "./lib/utils/structs/EnumerableMap.sol";
...
// contract ERC721 is Context, IERC721, IERC721Metadata, IERC721Enumerable {
...
contract UnilendV2Position is ERC721 {
    using SafeMath for uint256;
    ...
    mapping(uint => address) private nftPool;
    mapping(address => mapping(address => uint)) private userpoolNFT;
    ...
    function newPosition(address _pool, address _recipient) external onlyCore returns (uint nftID) {
        _tokenIdTracker.increment();
        nftID = _tokenIdTracker.current();

        nftPool[nftID] = _pool;
        userpoolNFT[_pool][_recipient] = nftID;
        _mint(_recipient, _tokenIdTracker.current());
    }

    function getNftId(address _pool, address _user) external view returns (uint nftID){
        return userpoolNFT[_pool][_user];
    }

    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override { 
        if(from != address(0)){
            require(userpoolNFT[nftPool[tokenId]][to] == 0, "UnilendV2: Position Already Exists");
            userpoolNFT[nftPool[tokenId]][from] = 0;
            userpoolNFT[nftPool[tokenId]][to] = tokenId;
        }
    }
    ...
    function position(uint _nftID) external view returns (nftPositionData memory _positionData){
